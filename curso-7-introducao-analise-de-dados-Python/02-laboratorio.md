...
def total_sales(n):
    ...
    total_sales = number_of_tickets * price_ticket
...
Eu aprendi que, para desenvolver a minha própria função, basta usar o termo def, seguido de um nome explicativo para a função e, em seguida, definir quais serão os seus parâmetros. Esses parâmetros serão processados no corpo da função, retornando um resultado com o termo return.
